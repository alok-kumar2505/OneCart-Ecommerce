...
function Edit() {
      ...
      try {
        ...
        if (response.data) {
          ...
          if (product) {
            ...
            setPrevImage1(product.image1 || '')
            setPrevImage2(product.image2 || '')
            setPrevImage3(product.image3 || '')
            setPrevImage4(product.image4 || '')
          } else {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(serverUrl + '/api/product/editproduct/' + id, formData, { withCredentials: true })
  ...
  const ImageUploader = ({ img, prevImg, setImg, id }) => (
    <label htmlFor={id} className="cursor-pointer group relative">
      <div className={`h-24 w-24 flex items-center justify-center overflow-hidden transition-all border ${!img && !prevImg ? 'border-dashed border-gray-400 bg-gray-50 hover:border-black hover:bg-gray-100' : 'border-gray-200'}`}>
        {!img && !prevImg ? (
          ...
          <img src={prevImg} alt="" className="h-full w-full object-cover" />
        )}
      </div>
        ...
        {!pageLoading && (
          <div className="max-w-4xl">
            <div className="mb-8 flex justify-between items-center">
              <div>
                <h1 className="font-playfair text-2xl font-bold text-black mb-2">Edit Product</h1>
                <p className="text-sm text-gray-500">Update the details below to edit this product in your catalog.</p>
              </div>
              <button onClick={() => navigate('/lists')} className="text-xs font-bold tracking-widest uppercase border border-gray-300 px-6 py-2 hover:bg-gray-50 transition-colors">
                Cancel
              </button>
            </div>
            ...
            <form onSubmit={onSubmitHandler} className="space-y-6">
              ...
              <div className="bg-white p-6 md:p-8 border border-gray-200 shadow-sm">
                ...
                <div className="flex gap-4 flex-wrap">
                  <ImageUploader img={image1} prevImg={prevImage1} setImg={setImage1} id="image1" />
                  <ImageUploader img={image2} prevImg={prevImage2} setImg={setImage2} id="image2" />
                  <ImageUploader img={image3} prevImg={prevImage3} setImg={setImage3} id="image3" />
                  <ImageUploader img={image4} prevImg={prevImage4} setImg={setImage4} id="image4" />
                </div>
              </div>
              ...
              <div className="bg-white p-6 md:p-8 border border-gray-200 shadow-sm">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                  <div>
                    ...
                    <select className="w-full border border-gray-300 px-5 py-3 text-sm focus:outline-none focus:border-black bg-white" onChange={(e) => setCategory(e.target.value)} value={category}>
                  ...
                  <div>
                    ...
                    <select className="w-full border border-gray-300 px-5 py-3 text-sm focus:outline-none focus:border-black bg-white" onChange={(e) => setSubCategory(e.target.value)} value={subCategory}>
